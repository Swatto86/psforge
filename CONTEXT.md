...
- Windows portable packaging uses `scripts/build-portable.ps1` and `portable.nsi` to extract the app and a SHA-256-pinned fixed WebView2 runtime. Release assets include checksums. Windows desktop tests attach to the launched webview; elevated runners use temporary app-scoped HKLM debugging policy because WebView2 150+ ignores environment debugging arguments at high integrity. Existing policy is restored afterward. Settings tests resolve the Windows known folder, back up existing PSForge settings, and restore them on teardown; overriding APPDATA alone does not redirect `dirs::config_dir()`.
