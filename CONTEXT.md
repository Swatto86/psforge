...
- Current version: **1.4.25**. Open a `.ps1` and press F5 to run it in the integrated console (same session as typing `& '.\script.ps1'` in VS Code / Windows Terminal). Untitled buffers still use the temp `psrun` wrapper. The AI tab can pick provider/model, including OpenCode + local Ollama.
