...
- [x] **S15-24** (LOW) — When a console's first shell finished starting, it took focus from wherever the user was: an open File menu snapped shut, or the caret left the editor. Seen as a Windows CI failure (Exit missing from the File menu). The first start now leaves focus alone if another control has it; restarts and run paths still focus the console.
