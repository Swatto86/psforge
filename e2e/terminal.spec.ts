...
async function waitForPrompt(timeoutMs = TERMINAL_READY_TIMEOUT): Promise<void> {
  await browser.waitUntil(
    async () => {
      const c = await getTerminalContent();
      return c.includes('PS') && c.includes('>');
    },
    { timeout: timeoutMs, interval: 300, timeoutMsg: `PS prompt not found (waited ${timeoutMs}ms)` }
  );
}
...
describe('Integrated Terminal', () => {
  ...
  describe('Command Execution', () => {
    ...
    it('cls clears the screen, removing prior content', async () => {
      ...
      await browser.waitUntil(
        ...
        { timeout: COMMAND_OUTPUT_TIMEOUT, interval: 300, timeoutMsg: 'BEFORE_CLS did not appear' }
      );
  ...
  describe('Command History', () => {
    ...
    it('Up arrow recalls the most recently executed command', async () => {
      ...
      await submitInput();
      await browser.waitUntil(
        async () => (await getTerminalContent()).includes('PSFORGE_HISTORY_SEED'),
        { timeout: COMMAND_OUTPUT_TIMEOUT, interval: 300, timeoutMsg: 'history seed command never ran' }
      );
      ...
      expect(await getLastInputLine()).toContain('Write-Host PSFORGE_HISTORY_SEED');
    });
  ...
  describe('CWD in Prompt', () => {
    ...
    it('prompt path updates after a Set-Location command', async () => {
      ...
      const beforePrompt = before.split('\n').reverse().find(l => l.includes('PS') && l.includes('>')) ?? '';
      ...
      await browser.waitUntil(
        async () => {
          const c = await getTerminalContent();
          const lines = c.split('\n').reverse();
          const latest = lines.find(l => l.includes('PS') && l.includes('>')) ?? '';
          return latest.toLowerCase().includes('temp') && latest !== beforePrompt;
        },
        { timeout: 5000, interval: 200, timeoutMsg: 'Prompt did not update to show $env:TEMP' }
      );

      const after = await getTerminalContent();
      const afterPrompt = after.split('\n').reverse().find(l => l.includes('PS') && l.includes('>')) ?? '';
